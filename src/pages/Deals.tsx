...
const Deals = () => {
  ...
  const totalPipelineValue = useMemo(() => {
    return dealLeads
      .filter((lead) => lead.status === 'Interested' || lead.status === 'Negotiating')
      .reduce((acc, lead) => acc + (lead.dealValue || 0), 0);
  }, [dealLeads]);

  const totalClosedValue = useMemo(() => {
    return dealLeads
      .filter((lead) => lead.status === 'Deal Closed')
      .reduce((acc, lead) => acc + (lead.dealValue || 0), 0);
  }, [dealLeads]);

  const interestedCount = dealLeads.filter((lead) => lead.status === 'Interested').length;
  const negotiatingCount = dealLeads.filter((lead) => lead.status === 'Negotiating').length;
  const closedCount = dealLeads.filter((lead) => lead.status === 'Deal Closed').length;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        <div className="neo-card p-6">
          ...
          <p className="text-xs uppercase tracking-[0.18em] text-[var(--text-secondary)] font-semibold mt-2">
            Interested
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredDeals.map((lead) => (
          <Link
            key={lead.id}
            to={`/leads/${lead.id}`}
            className="neo-card p-6 hover:-translate-y-1 transition-all duration-300"
          >
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              <div
                className={cn(
                  ...
                  statusStyles[lead.status] || 'bg-gray-500/10 text-gray-400 border-gray-400/20'
                )}
              >
                {lead.status}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-black/8 dark:border-white/8">
              <div>
                <p className="text-[10px] uppercase tracking-[0.18em] text-[var(--text-secondary)] font-bold mb-1">
                  Deal Value
                </p>
