...
export default function LeadGlobe() {
  ...
  return <main className="relative -mx-4 -mt-32 h-[100dvh] min-h-0 overflow-hidden bg-black text-white sm:-mx-6 lg:-mx-8 xl:-mx-10">
    ...
    <div className="absolute inset-0 z-10 flex items-center justify-center pt-32 sm:pt-36">
      <Globe ref={globeRef as any} width={typeof window !== 'undefined' ? Math.min(window.innerWidth - (window.innerWidth >= 1024 ? 320 : 32), 920) : 900} height={typeof window !== 'undefined' ? Math.min(window.innerHeight, 820) : 700} backgroundColor="rgba(0,0,0,0)" rendererConfig={{ antialias: false, alpha: true, powerPreference: 'low-power' }} globeImageUrl="https://unpkg.com/three-globe/example/img/earth-night.jpg" backgroundImageUrl="https://unpkg.com/three-globe/example/img/night-sky.png" polygonsData={countries} polygonCapColor={() => 'rgba(74, 92, 153, 0.16)'} polygonSideColor={() => 'rgba(255, 122, 0, 0.14)'} polygonStrokeColor={() => 'rgba(255,255,255,0.12)'} polygonsTransitionDuration={500} pointsData={filteredLeads} pointLat="latitude" pointLng="longitude" pointColor={(point: any) => point.id === selectedId ? '#ff7a00' : '#a99bff'} pointRadius={(point: any) => point.id === selectedId ? 0.65 : 0.42} pointAltitude={0.025} pointsMerge={false} onGlobeReady={() => setGlobeReady(true)} onPointClick={(point: any) => focusLead(point)} />
    </div>
