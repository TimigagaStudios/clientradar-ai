...
export default function DemoPreview() {
  ...
  return <main className="min-h-screen bg-[var(--bg)] text-[var(--text-primary)]">
    <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6"><Link to="/demos" className="inline-flex items-center gap-2 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)]"><ArrowLeft size={16} /> Back to Demo Factory</Link><span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-[var(--accent)]"><Sparkles size={14} /> Demo concept</span></nav>
    <section className="mx-auto max-w-6xl px-6 pb-16 pt-12"><p className="text-sm font-bold uppercase tracking-[0.2em] text-[var(--accent)]">{content.hero?.eyebrow || content.category}</p><h1 className="mt-4 max-w-4xl text-5xl font-black leading-tight md:text-7xl">{content.hero?.title || content.businessName}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--text-secondary)]">{content.hero?.description}</p><button className="btn-neumorph-primary mt-8 px-6 py-3">{content.hero?.cta || 'Get in touch'}</button></section>
    <section className="mx-auto grid max-w-6xl gap-5 px-6 pb-16 md:grid-cols-3">{(content.sections || []).map((section: any) => <article key={section.title} className="neo-card p-6"><CheckCircle2 className="mb-5 text-[var(--accent)]" size={24} /><h2 className="text-xl font-bold">{section.title}</h2><p className="mt-3 leading-7 text-[var(--text-secondary)]">{section.body}</p></article>)}</section>
    <section className="mx-auto max-w-6xl px-6 pb-20"><div className="neo-card grid gap-5 p-6 md:grid-cols-3"><div className="flex items-center gap-3"><MapPin className="text-[var(--accent)]" size={18} />{content.city || 'Local business'}</div>{content.contact?.phone && <div className="flex items-center gap-3"><Phone className="text-[var(--accent)]" size={18} />{content.contact.phone}</div>}{content.contact?.email && <div className="flex items-center gap-3"><Mail className="text-[var(--accent)]" size={18} />{content.contact.email}</div>}</div><p className="mt-4 text-center text-xs text-[var(--text-secondary)]">{content.disclaimer}</p></section>
  </main>;
}
