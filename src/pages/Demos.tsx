...
const Demos = () => {
  const { leads, updateDemoStatus, refreshLeads } = useLeads() as any;
  ...
  const handleCreateDemo = async () => {
    ...
    try {
      ...
      const processResponse = await fetch('/api/demo-jobs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jobId: result.data.id, action: 'process' }) });
      const processResult = await parseDemoResponse(processResponse);
      if (!processResponse.ok) throw new Error(processResult.error || 'Demo processing failed.');
      await refreshLeads();
      await refreshJobs();
      setSelectedLeadId('');
    } catch (error) {
  ...
  const retryDemo = async (jobId: string) => {
    setJobError('');
    try {
      const response = await fetch('/api/demo-jobs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jobId, action: 'process' }) });
      const result = await parseDemoResponse(response);
